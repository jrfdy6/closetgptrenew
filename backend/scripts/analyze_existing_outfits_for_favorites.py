...
# Import the analytics functions directly to avoid circular imports
...
def get_user_favorites(user_id: str, item_type: Optional[str] = None, limit: int = 10):
    """Get user's favorite items, optionally filtered by type."""
    try:
        # Query favorite scores
        query = db.collection("item_favorite_scores").where("user_id", "==", user_id)
        ...
        for doc in score_docs:
            ...
            # Get item details
            item_ref = db.collection("wardrobe").document(score_data["item_id"])
