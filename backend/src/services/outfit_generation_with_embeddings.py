#!/usr/bin/env python3
"""
Outfit Generation with Vector Embeddings Integration
===================================================

This service integrates vector embeddings and personalized recommendations
with the existing outfit generation pipeline to provide highly personalized
outfit suggestions.

Features:
- Seamless integration with existing outfit generation
- Personalized ranking of generated outfits
...
from .embedding_service import EmbeddingService, UserInteraction
from .personalized_recommendation_engine import PersonalizedRecommendationEngine, RecommendationStrategy
...
class OutfitGenerationWithEmbeddings:
    """
    Enhanced outfit generation with vector embeddings and personalization
    """
    
    def __init__(self, openai_api_key: Optional[str] = None):
        # Initialize services
        self.embedding_service = EmbeddingService(openai_api_key)
        self.recommendation_engine = PersonalizedRecommendationEngine(self.embedding_service)
        ...
        logger.info("✅ Outfit Generation with Embeddings initialized")
    ...
    async def generate_personalized_outfits(
        ...
        """
        Generate personalized outfits using vector embeddings
        ...
        try:
            logger.info(f"🎯 Starting personalized outfit generation for user {user_id}")
            ...
            logger.info(f"⏱️ Personalized outfit generation completed in {generation_time:.2f}s")
    ...
    async def _apply_personalization(
        ...
        try:
            ...
            # Convert to PersonalizedOutfitResult
            return PersonalizedOutfitResult(
    ...
    def update_personalization_settings(
        ...
        logger.info(f"✅ Updated personalization settings: enabled={self.enable_personalization}, max_outfits={self.max_personalized_outfits}")
