...
# Subscription tier configuration for OpenAI flat lay usage.
DEFAULT_SUBSCRIPTION_TIER = "tier1"

# None indicates unlimited usage for that tier.
...
def subscription_defaults(
    ...
    """Generate default subscription fields for a user document using new Stripe payment system schema."""
...
def quotas_defaults(
    ...
    """Generate default quotas for a user document using new Stripe payment system schema."""
    now = now or datetime.now(timezone.utc)
    now_timestamp = int(now.timestamp())
    limit = TIER_LIMITS.get(tier, 1)
