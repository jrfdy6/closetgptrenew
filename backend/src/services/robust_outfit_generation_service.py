...
class RobustOutfitGenerationService:
    ...
    async def _generate_outfit_internal(self, context: GenerationContext, session_id: str) -> OutfitGeneratedOutfit:
        ...
        # ═══════════════════════════════════════════════════════════
        # PROGRESSIVE TIER FILTER (RUNS FIRST - Before Occasion Filter!)
        # ═══════════════════════════════════════════════════════════
        # For tier-based occasions (Interview, Business, etc.), apply progressive tier filtering
        ...
        # ═══════════════════════════════════════════════════════════
        # STEP 1: OCCASION-FIRST FILTERING (Skipped for tier-based occasions)
        # ═══════════════════════════════════════════════════════════
        
        logger.info(f"🎯 STEP 1: Occasion-First Filtering")
        
        # CRITICAL: Skip occasion filter if tier filter was applied (it already did the filtering)
        if progressive_filter_applied:
            logger.info(f"✅ STEP 1 SKIPPED: Tier filter already applied, using {len(context.wardrobe)} items")
            occasion_candidates = context.wardrobe
        elif context.base_item_id:
    ...
    def _apply_progressive_interview_business_filter(self, wardrobe, context, recently_used_item_ids):
        """
        Apply progressive tier filtering for occasions with formality requirements.
        
        Now uses the comprehensive FormalityTierSystem for all occasions that need it.
        ...
        """
        # Check if this occasion needs tier filtering
        if not self.tier_system.should_apply_tier_filter(context.occasion):
            return wardrobe
        
        # Apply progressive tier filter
        filtered_wardrobe, tier_used = self.tier_system.apply_progressive_filter(
            wardrobe=wardrobe,
            occasion=context.occasion,
            style=context.style,
        ...
        logger.info(f"✅ TIER FILTER: Applied {tier_used.value} for {context.occasion}")
    ...
    def _hard_filter(self, item: ClothingItem, occasion: str, style: str) -> bool:
        ...
        Now uses modular occasion filters for better maintainability.
        """
        # Use the new modular occasion filters
