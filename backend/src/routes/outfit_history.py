...
async def calculate_worn_outfits_this_week(user_id: str) -> int:
    ...
    try:
        ...
        history_ref = db.collection('outfit_history').filter('user_id', '==', user_id)
        total_worn = 0
...
async def get_todays_outfit_suggestion(
    ...
    try:
        ...
        query = suggestions_ref.filter('user_id', '==', current_user.id).filter('date', '==', today_str)
...
async def debug_user_outfit_history(
    ...
    try:
        ...
        for collection_name in collections_to_check:
            ...
            query = collection_ref.filter("user_id", "==", user_id)
            docs = query.stream()
...
async def verify_worn_calculation(
    ...
    try:
        ...
        outfits_ref = db.collection('outfits').filter('user_id', '==', user_id)
        outfits_count = 0
        outfits_found = []
