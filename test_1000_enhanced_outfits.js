#!/usr/bin/env node
/**
 * COMPREHENSIVE 1000-OUTFIT TEST WITH ENHANCED VALIDATION
 * Test the enhanced item type matching system with comprehensive attribute analysis
 */
...
async function makeRequest(url, options = {}) {
    return new Promise((resolve, reject) => {
        const req = https.request(url, options, (options.headers && options.headers['Content-Length']) ? options : {...options, headers: {...options.headers}}, (res) => {
            let data = '';
...
function generateRandomWardrobe() {
    ...
    const occasions = [
        'casual', 'business', 'formal', 'athletic', 'evening', 'party', 'workout'
    ];
...
function generateRandomRequest() {
    ...
    return {
        ...
        userProfile: { 
            ...
            style: styles[Math.floor(Math.random() * styles.length)],
            preferences: {
                colorPalette: ['black', 'white', 'blue', 'navy'],
                formality: Math.random() > 0.5 ? 'formal' : 'casual'
            }
        },
        style: styles[Math.floor(Math.random() * styles.length)],
...
function analyzeOutfit(outfit) {
    ...
    if (analysis.hasFormalShoes && analysis.hasTShirt) {
        analysis.inappropriateCombinations.push('formal_shoes_with_t_shirt');
    }
...
async function test1000Outfits() {
    console.log("🎯 COMPREHENSIVE 1000-OUTFIT TEST WITH ENHANCED VALIDATION");
    console.log("=" * 80);
    console.log("Testing enhanced item type matching with comprehensive attribute analysis...\n");
    ...
    for (let batch = 0; batch < Math.ceil(totalTests / batchSize); batch++) {
        const batchStart = batch * batchSize;
        const batchEnd = Math.min(batchStart + batchSize, totalTests);
        const currentBatchSize = batchEnd - batchStart;
        
        console.log(`🔄 Running batch ${batch + 1}/${Math.ceil(totalTests / batchSize)} (tests ${batchStart + 1}-${batchEnd})`);
        ...
        for (let i = 0; i < currentBatchSize; i++) {
            ...
            const promise = makeRequest(`${BACKEND_URL}/api/outfits/generate`, {
                method: 'POST',
                headers: {
            ...
            }).then(response => ({ index: batchStart + i, response }))
            .catch(error => ({ index: batchStart + i, error }));
        ...
        batchResults.forEach(({ index, response, error }) => {
            results.total++;
            ...
            if (response.status !== 200) {
                results.failed++;
                console.log(`   ❌ Test ${index + 1}: HTTP ${response.status}`);
                return;
            }
            ...
            if ((index + 1) % 100 === 0) {
                ...
                console.log(`   📊 Progress: ${index + 1}/${totalTests} | Success: ${successRate}% | Inappropriate: ${inappropriateRate}% | Missing: ${missingRate}% | Violations: ${violationRate}%`);
            }
        });
    ...
    const avgTops = Math.round(results.categoryDistribution.tops.reduce((a, b) => a + b, 0) / results.categoryDistribution.tops.length * 10) / 10;
    const avgBottoms = Math.round(results.categoryDistribution.bottoms.reduce((a, b) => a + b, 0) / results.categoryDistribution.bottoms.length * 10) / 10;
    const avgShoes = Math.round(results.categoryDistribution.shoes.reduce((a, b) => a + b, 0) / results.categoryDistribution.shoes.length * 10) / 10;
    const avgAccessories = Math.round(results.categoryDistribution.accessories.reduce((a, b) => a + b, 0) / results.categoryDistribution.accessories.length * 10) / 10;
    
    // Final Results
    console.log("\n🏆 COMPREHENSIVE 1000-OUTFIT TEST RESULTS");
    console.log("=" * 80);
