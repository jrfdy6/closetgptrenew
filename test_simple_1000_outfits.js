#!/usr/bin/env node
/**
 * SIMPLE 1000-OUTFIT TEST WITH ENHANCED VALIDATION
 * Test the enhanced item type matching system with a simpler approach
 */
...
async function test1000Outfits() {
    console.log("🎯 SIMPLE 1000-OUTFIT TEST WITH ENHANCED VALIDATION");
    console.log("=" * 80);
        ...
        for (let i = 0; i < currentBatchSize; i++) {
            ...
            try {
                ...
                if (response.status !== 200) {
                    results.failed++;
                    console.log(`   ❌ Test ${testIndex + 1}: HTTP ${response.status}`);
                    continue;
                }
                ...
                if ((testIndex + 1) % 50 === 0) {
                    ...
                    console.log(`   📊 Progress: ${testIndex + 1}/${totalTests} | Success: ${successRate}% | Inappropriate: ${inappropriateRate}% | Missing: ${missingRate}% | Violations: ${violationRate}%`);
                }
                
            } catch (error) {
                results.total++;
                results.failed++;
                console.log(`   ❌ Test ${testIndex + 1}: ERROR - ${error.message}`);
            }
